        ...
        <div class="card-body">
            <form action="{{ route('user.update', $user->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" style="width:650px" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name', $user->name) }}">
                </div>
                <div class="form-group">
                    <label class="font-weight-bold">Email</label>
                    <input type="text" style="width:650px" class="form-control @error('email') is-invalid @enderror" name="email" value="{{ old('email', $user->email) }}">
                </div>
                <div class="form-group">
                    <label class="font-weight-bold">Role User</label>
                    <br>
                    <select name="role" id="role" class="btn btn-light" data-toggle="dropdown" style="width:650px">
                        @foreach ($role as $rol)
                        <option value="{{ $rol->role }}" selected>{{ $rol->role }}</option>
                        @endforeach
                    </select>
                </div>
                <br>
